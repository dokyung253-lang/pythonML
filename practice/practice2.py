...
target_iris = target_iris[["PetalLengthCm", "PetalWidthCm"]]
print( target_iris )

# [단계 4] 학습 데이터 구성
# 꽃잎 길이와 꽃잎 너비를 이용하여  [길이, 너비] 형태의 2차원 리스트 iris_data를 생성하세요.
import numpy as np
length_Width = np.column_stack(( target_iris['PetalLengthCm'], target_iris['PetalWidthCm']))
print( length_Width )

# [단계 5] 정답 데이터(Target) 생성
# Iris-setosa : 1 # Iris-versicolor : 0 # 으로 구성된 iris_target 리스트를 생성하세요.
iris_data = np.concatenate( ( np.ones(150 ), np.zeros(150) ) )
print( iris_data )
