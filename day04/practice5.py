...
# 독립변수 (특성), 종속변수(타겟)
# study_hours,attendance,sleep_hours,internet_usage,assignments_completed,previous_score,placement_status
# exam_score
...
# [2] 모델 전수 탐색
# 다항 확장 : 특성(자료)들 간에 직선관계 드물다(단순회귀), 물고기길이, 물고기길이 제곱, 물고기길이 세제곱 ~(다항회귀)
# 직선관계가 아닌 곡선관계 만들고 다양한 경우에 수 학습자료 만든다. 주의할 점 : 과적합
...
for degree in [1,2,3,4,5] :
    poly = PolynomialFeatures( degree= degree, include_bias= False ) # degree : 차수, 1~5 반복
    ...
    # print( f'{ degree } 차수의 선형회귀 결정계수 : { r2 }')  # 결정계수 : 해당 모델이 예측한 결과가 얼마나 잘 설명되는지 수치화
...
# [3] 최적 모델 찾기위한 리스트에서 결정계수('r2')가 가장 큰 모델 찾기
...
def exam_score_predict( study_hours, attendance, sleep_hours, internet_usage, assignments_completed, previous_score  ) : # 매개변수에 *한번에 여러 변수들을 튜플 받을 수 있다. # 키워드
    ...
    return result[0] # 예측한 값을 반환한다. # 항상 예측 결과도 리스트 반환하기 떄문에 인덱스[0] 으로 하나 추출
...
result2 = exam_score_predict(2, 60, 5, 9, 4, 50)
print( f'result : {result}점')